...
func Usage() {
	...
	fmt.Fprintf(os.Stderr, "\tenumgen [flags] -type T files... # Must be a single package\n")
	fmt.Fprintf(os.Stderr, "For more information, see:\n")
	fmt.Fprintf(os.Stderr, "\thttps://pkg.go.dev/github.com/yikakia/enumgen\n")
...
func main() {
	...
	for _, pkg := range pkgs {
		...
		g.Printf("//go:generate go run -v -mod=mod github.com/yikakia/enumgen %s\n", strings.Join(os.Args[1:], " "))
		g.Printf("// Code generated by \"go run -v -mod=mod github.com/yikakia/enumgen %s\"; DO NOT EDIT.\n", strings.Join(os.Args[1:], " "))
		g.Printf("\n")
...
func (g *Generator) generate(typeName string, values []Value) {
	...
	g.Printf("\t// Re-run the enumgen command to generate them again.\n")
	...
	g.buildConvs(typeName)
}

func (g *Generator) buildConvs(typeName string) {
	...
	for _, convType := range convTypes {
		toTypeFuncName := strings.ToUpper(convType[:1]) + convType[1:]

		g.Printf("func (i %[1]s) %[2]s () %[3]s {\n", typeName, toTypeFuncName, convType)
		g.Printf("return %s(i)\n", convType)
		g.Printf("}\n")
		g.Printf("\n")
	}
