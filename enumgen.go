...
func (g *Generator) buildConvs(typeName string) {
	...
	firstRune := strings.ToLower(typeName)[0]

	build := func(toType, toTypeFuncName string) {
		g.Printf("func (%[1]c %[2]s) %[3]s () %4s {\n", firstRune, typeName, toTypeFuncName, toType)
		g.Printf("return %s(%c)\n", toType, firstRune)
		g.Printf("}\n")
		g.Printf("\n")
	}
	...
	for _, convType := range convTypes {
		build(convType, strings.ToUpper(convType[:1])+convType[1:])
	}
}
